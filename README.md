...
1. Ensure you have run all migrations with `python tasks.py db:dev:upgrade`
The next user manual steps will be carried out using the Interactive Backend Docs.
1. Go to `http://localhost:8000/docs`

**Part A: Creating an Account**
1. Go to the users section and expand the "Create User" section
2. Click on "Try it out" 
4. Replace the text field with the information for your user, then click "Execute"
5. Remember your `user` id as it will be used in the subsequent steps

**Part B: Creating an API key**
3. Now, go to the `api-keys` section.
4. Expand the "Generate API Key" request
7. Click on "Try it out"
8. Replace the `user_id` field value the id of the account you just created.
9. Click on "Execute" and notice a successful response with the `key` and your `user_id`. Again copy this key to use in the next step

**Part C: Using a route secured by an API key**
1. In the 'api-keys' section, go to the 'Read Api Key Protected Route'
2. Run the following from your terminal:

```bash
curl -X 'GET' 'http://localhost:8000/api-keys/protected' -H 'accept: application/json' -H 'x-api_key: <api-key-you-generated>'
```
3. Notice an "Authorized!" message
4. Now for the unauthorized case, run any of the following commands for api keys that do not exist:
```bash
curl -X 'GET' 'http://localhost:8000/api-keys/protected' -H 'accept: application/json'
```
OR
```bash
curl -X 'GET' 'http://localhost:8000/api-keys/protected' -H 'accept: application/json' -H 'x-api_key: key-that-does-not-exist'
```
