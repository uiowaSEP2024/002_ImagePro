...
def test_requests_made_with_api_key():
    with requests_mock.Mocker() as m:
        ...
        m.post(tracker.urls.jobs_url,
               json={"provider_job_id": "1", "customer_id": 1, "provider_job_name": "test_job"})

        tracker.create_job("1", 1, "test_job")

        assert tracker.api_key == m.last_request.headers[TrackerApi.HTTP_API_KEY_HEADER_KEY], \
...
def test_job():
    ...
    responses.add(
        url=tracker.urls.jobs_config_url,
    ...
    responses.add(
        url=tracker.urls.jobs_url,
        method=responses.POST,
        json={"provider_job_id": "1"}
    )

    tracker_jobapi = tracker.create_job("1", 1, "test_job")
    assert tracker_jobapi.provider_job_id == "1", "Expected TrackerJobApi to set the returned job_provider_id on itself"

    responses.add(
        url=tracker.urls.events_url,
        method=responses.POST,
        json={"id": 1}
    )
    tracker_eventapi = tracker_jobapi.send_event(kind="step", name="My Event")
    assert tracker_eventapi.event_id == 1, "Expected TrackerEventApi to set the returned event id on itself"
