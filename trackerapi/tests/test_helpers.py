...
def test_job_config_manager_init_from_configs():
    job_config_manager = JobConfigManager(configs=[test_job_config])
    ...
    assert (
        "test_job" in job_config_manager.tags
...
def test_job_config_manager_init_from_config_file():
    ...
    assert (
        config_result.tag == "test_schema_job"
    ), "Expected config tag to be same as provided in configuration file"
    assert (
        config_b_result.tag == "test_schema_job_b"
    ), "Expected config tag to be same as provided in configuration file"
    ...
    assert (
        "test_schema_job" in job_config_manager.tags
    ), "Expected config's tag to be in config manager's tags"
    assert (
        "test_schema_job_b" in job_config_manager.tags
    ), "Expected config's tag to be in config manager's tags"
