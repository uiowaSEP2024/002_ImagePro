...
def test_create_api_key(db, random_test_user):
    api_key = services.create_apikey_for_user(db=db, user_id=random_test_user.id, key=schemas.ApikeyCreate(note="key"))
...
def test_get_api_keys_for_user(db, random_test_user):
    api_key1 = services.create_apikey_for_user(db, random_test_user.id, key=schemas.ApikeyCreate(note="key"))
    api_key2 = services.create_apikey_for_user(db, random_test_user.id, key=schemas.ApikeyCreate(note="key"))
