...
def test_create_api_key(db, random_test_admin_user):
    api_key = services.create_apikey_for_user(
        db=db, user_id=random_test_admin_user.id, key=schemas.ApikeyCreate(note="key")
    )

    assert isinstance(api_key, models.Apikey)
    assert len(api_key.key) >= API_KEY_LENGTH
    assert api_key.user_id == random_test_admin_user.id
    assert api_key.created_at is not None

    api_keys_for_user = services.get_api_keys_for_user(db, random_test_admin_user.id)
    ...
    assert user_from_api_key.id == random_test_admin_user.id


def test_get_api_keys_for_user(db, random_test_admin_user):
    ...
    assert api_key1.user_id == random_test_admin_user.id
    assert api_key2.user_id == random_test_admin_user.id
...
def test_get_user_from_api_key(db, random_test_admin_user):
    api_key = services.create_apikey_for_user(
        db, random_test_admin_user.id, key=schemas.ApikeyCreate(note="key")
    )
    user_from_api_key = services.get_user_from_apikey_key(db, api_key.key)
